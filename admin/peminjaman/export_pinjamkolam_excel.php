...
$id_kolam_f  = isset($_GET['id_kolam']) ? trim($_GET['id_kolam']) : '';


/* =========================
   Query data pinjamkolam
========================= */
$sql = "SELECT pk.*,
               u.nama_lengkap,
               k.jenis_kolam,
...
if ($keyword !== '') {
    $sql .= " AND (pk.nama LIKE ? OR u.nama_lengkap LIKE ? OR k.jenis_kolam LIKE ?)";
    $kw = "%{$keyword}%";
...
$filename = 'laporan_peminjaman_kolam_' . date('Ymd_His') . '.xls';
...
echo '<x:Name>Laporan Peminjaman Kolam</x:Name>';
...
echo '<h2 style="text-align:center;">Laporan Peminjaman Kolam Renang</h2>';
...
echo '<th style="width:120px;">Kolam</th>';
...
while ($d = $res->fetch_assoc()) {
    ...
    $kolam = cleanExcel($d['jenis_kolam']);
    $kelas = cleanExcel($d['nama_kelas']);
    $tujuan = !empty($d['tujuan']) ? cleanExcel($d['tujuan']) : '-';
    
    $pic = cleanExcel($d['nama_lengkap']);
    if (!empty($d['nama']) && $d['nama'] !== $d['nama_lengkap']) {
        $pic .= ' - ' . cleanExcel($d['nama']);
    }
    ...
    echo '<td>' . htmlspecialchars($kolam, ENT_QUOTES, 'UTF-8') . '</td>';
    echo '<td>' . htmlspecialchars($kelas, ENT_QUOTES, 'UTF-8') . '</td>';
