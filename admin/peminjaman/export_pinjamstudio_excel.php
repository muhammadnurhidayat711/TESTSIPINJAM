...
$filename = 'laporan_peminjaman_studio_' . date('Ymd_His') . '.xls';
...
echo '<x:Name>Laporan Peminjaman Studio</x:Name>';
...
echo '<h2 style="text-align:center;">Laporan Peminjaman Studio</h2>';
...
echo '<th style="width:120px;">Studio</th>';
...
while ($d = $res->fetch_assoc()) {
    ...
    $studio = cleanExcel($d['jenis_studio']);
    $kelas = cleanExcel($d['nama_kelas']);
    $tujuan = !empty($d['tujuan']) ? cleanExcel($d['tujuan']) : '-';
    
    // PIC: nama_lengkap (user) + nama peminjam (pinjamstudio) dengan spasi
    ...
    echo '<td>' . htmlspecialchars($studio, ENT_QUOTES, 'UTF-8') . '</td>';
