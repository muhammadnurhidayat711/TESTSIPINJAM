...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['simpan'])) {
  // isi $old dari POST (riwayat isian)
  ...
  if (!$posted_token || !$csrf_token || !hash_equals($csrf_token, $posted_token)) {
    ...
    if ($id_user <= 0) {
      ...
      // Jika pilih meja/kursi "Iya" tapi jumlah kosong
      ...
      // Jika pilih "Tidak", kosongkan jumlah
      if ($meja !== 'Iya')
        $jumlah_meja_in = '';
      if ($kursi !== 'Iya')
        $jumlah_kursi_in = '';
      ...
      // Ambil data barang (untuk pesan & insert)
      ...
      if ($can_proceed) {
        ...
        if (!$result_cek) {
              ...
              if ($is_recurring === 'yes' && !empty($recurring_days)) {
                ...
                if (!empty($overlap_days)) {
                  ...
                  if (!($new_time_end <= $existing_time_start || $new_time_start >= $existing_time_end)) {
                    ...
                    // ✅ UBAH: Format 24-jam tanpa AM/PM
          ...
          if ($ada_konflik) {
            ...
            // Notifikasi ke user bersangkutan + admin
      ...
      if ($can_proceed) {
        ...
        if (mysqli_query($conn, $query_insert)) {
          ...
          if ($is_recurring === 'yes') {
            ...
            $recurring_info = "\\\\n\\\\n✓ Peminjaman rutin setiap: " . implode(', ', $day_labels);
          }
    ...
    if (window.Swal) {
        Swal.fire({
            ...
            html: `<div style='text-align:left; white-space:pre-line'>Peminjaman " . addslashes($data_barang['nama_barang']) . " berhasil ditambahkan.\\n⏳ Status: <strong>Menunggu Persetujuan Admin</strong>\\n" . $recurring_info . "\\n\\n📧 Notifikasi telah dikirim ke admin untuk ditinjau.</div>`
        }).then(() => {
            window.location.href = '?view=datapinjambarang';
        });
    } else {
        alert('BERHASIL! " . addslashes($data_barang['nama_barang']) . " berhasil ditambahkan.\\\\n⏳ Status: Menunggu Persetujuan Admin\\\\n" . $recurring_info . "\\\\n\\\\n📧 Notifikasi telah dikirim ke admin.');
        window.location.href = '?view=datapinjambarang';
    }
</script>";
