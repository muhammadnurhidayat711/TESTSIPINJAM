<?php
// ========================================
// CREATE PEMINJAMAN KOLAM — SIPINJAM
// ========================================
...
if (isset($_POST['simpan'])) {
    ...
    if (!empty($errors)) {
        ...
        // Ambil info kolam dan kelas yang dipilih
        $query_kolam_dipilih = mysqli_query($conn, "SELECT * FROM kolam WHERE id_kolam = '$id_kolam'");
        $kolam_dipilih = mysqli_fetch_assoc($query_kolam_dipilih);
        ...
        $cek_jadwal = mysqli_query($conn, "
            SELECT pk.*, k.jenis_kolam, kl.nama_kelas, u.nama_lengkap
            FROM pinjamkolam pk
            LEFT JOIN kolam k ON pk.id_kolam = k.id_kolam
            LEFT JOIN kelas kl ON pk.id_kelas = kl.id_kelas
            LEFT JOIN user u ON pk.id_user = u.id
            WHERE pk.id_kolam = '$id_kolam' 
        ...
        if (mysqli_num_rows($cek_jadwal) > 0) {
            ...
            $info_kolam = htmlspecialchars($data_bentrok['jenis_kolam']);
            ...
            $info_penanggung_jawab = !empty($data_bentrok['nama']) ? htmlspecialchars($data_bentrok['nama']) : "-";
            ...
            // Cari alternatif kolam yang tersedia
            $query_alternatif = mysqli_query($conn, "
                SELECT k.* FROM kolam k
                WHERE k.id_kolam NOT IN (
                    SELECT pk.id_kolam FROM pinjamkolam pk
            ...
            if (mysqli_num_rows($query_alternatif) > 0) {
                $list_alternatif = "<hr><p><b>🏊 Kolam Tersedia:</b></p><ul>";
                while ($alt = mysqli_fetch_assoc($query_alternatif)) {
                    $nama_alt = htmlspecialchars($alt['jenis_kolam']);
                    $list_alternatif .= "<li>" . $nama_alt . "</li>";
                }
                $list_alternatif .= "</ul>";
            } else {
                $list_alternatif = "<hr><p style='color:#d33;'><b>⚠️ Semua kolam tidak tersedia pada jadwal ini.</b></p>";
            }

            $html_content = '<div style="text-align: left;">'.
                        '<p><strong style="color:#d33;font-size:16px;">❌ JADWAL BENTROK!</strong></p>'.
                        '<p>Kolam sudah dipinjam pada waktu tersebut.</p><hr>'.
                        '<table style="width:100%;border-collapse:collapse;">'.
                        '<tr><td style="padding:8px;background:#f5f5f5;width:35%;"><b>🏊 Kolam</b></td><td style="padding:8px;">'.$info_kolam.'</td></tr>'.
                        ...
                        '<tr><td style="padding:8px;background:#f5f5f5;"><b>👨‍💼 Penanggung Jawab</b></td><td style="padding:8px;">'.$info_penanggung_jawab.'</td></tr>'.
            ...
            $alert_script = "
                Swal.fire({
                    ...
                    confirmButtonText: 'Ubah Jadwal/Kolam',
                    width: '800px'
                });
            ";
        } else {
            ...
            if (mysqli_query($conn, $query)) {
                $id_pinjam_kolam = mysqli_insert_id($conn);
                ...
                while ($lu = mysqli_fetch_assoc($loop_user)) {
                    mysqli_query($conn, "
                        INSERT INTO `notice3` 
                        SET `id_pinjamkolam` = $id_pinjam_kolam, 
                            `id_user` = " . $lu['id'] . ", 
                            `waktu` = " . time() . ", 
                            `status` = 0
                    ");
                ...
                // 3. Get data kolam dan kelas
                $jenis_kolam = $kolam_dipilih['jenis_kolam'] ?? 'Kolam';
                ...
                $fcm_title = "🏊 Peminjaman Kolam Baru!";
                $fcm_body = "$nama_peminjam mengajukan peminjaman $jenis_kolam ($nama_kelas) pada $tgl_indo ($waktu_info)";
            
                $fcm_data = [
                    'booking_id' => (string)$id_pinjam_kolam,
                    'type' => 'new_pool_booking',
                    'kolam' => $jenis_kolam,
                    'kelas' => $nama_kelas,
                    'tanggal' => $tgl_indo,
                    'waktu' => $waktu_info,
                ...
                $html_success = '<div style="text-align: left; line-height: 1.6;">'.
                            ...
                            '<div style="font-size: 16px; font-weight: 700; color: #047857; margin-bottom: 12px;">📋 Detail Peminjaman Kolam</div>'.
                            '<table style="width: 100%; font-size: 14px;">'.
                            '<tr><td style="padding: 6px 0; color: #065f46; font-weight: 600; width: 40%;">ID Booking</td><td style="padding: 6px 0;">: <code style="background: #fff; padding: 4px 8px; border-radius: 6px; font-weight: 700; color: #047857;">#'.$id_pinjam_kolam.'</code></td></tr>'.
                            '<tr><td style="padding: 6px 0; color: #065f46; font-weight: 600;">🏊 Kolam</td><td style="padding: 6px 0;">: <strong>'.htmlspecialchars($jenis_kolam).'</strong></td></tr>'.
                            ...
                            '<tr><td style="padding: 6px 0; color: #065f46; font-weight: 600;">👨‍💼 Penanggung Jawab</td><td style="padding: 6px 0;">: '.htmlspecialchars($nama).'</td></tr>'.
                ...
                $alert_script = "
                    // ✅ Log ke Console
                    console.group('🏊 PEMINJAMAN KOLAM BERHASIL');
                    console.log('%c✅ Booking ID: #$id_pinjam_kolam', 'color: #10b981; font-weight: bold; font-size: 16px');
                    console.log('🏊 Kolam:', ".json_encode($jenis_kolam).");
                    console.log('📚 Kelas:', ".json_encode($nama_kelas).");
                    console.log('👤 Peminjam:', ".json_encode($nama_peminjam).");
                    console.log('👨‍💼 Penanggung Jawab:', ".json_encode($nama).");
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            window.location.href = '?view=datapinjamkolam';
                        } else if (result.dismiss === Swal.DismissReason.cancel) {
                            window.location.href = '?view=createpinjamkolam';
                        }
                    });
                ";
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="card-title">🏊 Create Peminjaman Kolam</div>
                </div>
                <div class="card-body">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_kolam" class="form-control" required>
                                    ...
                                    while($kolam = mysqli_fetch_array($kolam_query)) {
                                        $selected = ($form_data['id_kolam'] == $kolam['id_kolam']) ? 'selected' : '';
                                        echo "<option value='{$kolam['id_kolam']}' $selected>{$kolam['jenis_kolam']}</option>";
                                    }
                                    ?>
                                </select>
                            </div>
                            ...
                            <div class="form-group full-width">
                                ...
                                <textarea name="tujuan" 
                                          class="form-control" 
                                          placeholder="Jelaskan tujuan peminjaman kolam..." 
                                          rows="3"><?php echo htmlspecialchars($form_data['tujuan'], ENT_QUOTES, 'UTF-8'); ?></textarea>
                                <small class="form-hint">
